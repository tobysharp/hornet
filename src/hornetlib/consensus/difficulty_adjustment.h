...
[[nodiscard]] inline protocol::CompactTarget AdjustCompactTarget(
    int height, const protocol::BlockHeader& parent, const HeaderAncestryView& view) {
  return IsDifficultyTransition(height)
             ? ComputeCompactTarget(
                   height, parent.GetCompactTarget(),
                   view.TimestampAt(height - constants::kBlocksPerDifficultyPeriod),
                   parent.GetTimestamp())
             : parent.GetCompactTarget();
}
